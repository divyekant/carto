...
func GenerateCLAUDE(input Input) string {
	...
	b.WriteString("After editing files, update the Carto index:\n\n")
	b.WriteString("```bash\n")
	fmt.Fprintf(&b, "carto writeback %s --file <changed-file>\n", input.ProjectName)
	b.WriteString("```\n\n")
	b.WriteString("This re-analyzes the file, supersedes old atoms, and updates graph links.\n")
	b.WriteString("The index stays current without a full re-index.\n\n")
...
func GenerateCursorRules(input Input) string {
	...
	b.WriteString("Write back after changes:\n")
	fmt.Fprintf(&b, "  carto writeback %s --file <changed-file>\n\n", input.ProjectName)
	b.WriteString("This re-analyzes the file, supersedes old atoms, and updates graph links.\n")
